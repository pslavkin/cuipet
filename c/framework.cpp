	...
	void 	Framework::Select_Sheet(unsigned char Sheet)
{
	...
	for(i=0;i<Sheet;i++) {	//para los sheets menores al seleccionado
			...
			//para el seleccionado
 			...
 			Inst_Buf[i]->Set_Pos(HEAD_HEIGHT*(MAX_SHEETS-i-1)+1,1);
 			Inst_Buf[i]->Set_Size(Menu::Main_Sheet->Max_Y()-i*HEAD_HEIGHT-2-HEAD_HEIGHT*(MAX_SHEETS-i-1),Menu::Main_Sheet->Max_X()-2);
